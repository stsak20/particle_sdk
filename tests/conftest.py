...
from particle_sdk import ParticleSDK, AsyncParticleSDK, DefaultAioHttpClient
from particle_sdk._utils import is_dict
...
logging.getLogger("particle_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[ParticleSDK]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with ParticleSDK(base_url=base_url, jwt_token=jwt_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncParticleSDK]:
    ...
    async with AsyncParticleSDK(
        base_url=base_url, jwt_token=jwt_token, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
